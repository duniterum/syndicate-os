...
| Q11 | **HEADER DEFECT → FIXED (built, awaiting push).** Added a secondary "Member" button (wallet icon) beside the join CTA (desktop + mobile sheet) → `/member` read-only self-readback. Truthful: connect + read your standing, NOT "sign in" (SIWE session stays dark until auth go-live). Investigated: wallet CONNECT works today; only the persistent verify is withheld (`RainbowKitRoot.tsx:64-72`). | ⏳ built, awaiting push GO | founder reviews diff | `components/layout/PublicLayout.tsx` |
