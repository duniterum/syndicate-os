...
- [x] **Header member affordance** (`components/layout/PublicLayout.tsx`, Q11) : bouton secondaire **« Member »** (icône wallet) à côté du CTA join, desktop + sheet mobile → `/member` (self-readback **lecture seule**) ; distingue membre-existant de nouveau-entrant ; **jamais « sign in »** (la session SIWE reste dark jusqu'au go-live auth — s'allume alors sans changement de code). Tokens only, **+0 couleur**.
