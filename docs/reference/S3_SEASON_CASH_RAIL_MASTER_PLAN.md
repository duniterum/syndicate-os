...
|---|---|---|---|
| **S3-0** ✅ (partial: copy `de4c8b7`, deps+cfg `f036419`; tool-probe done — halmos path recorded) | TRUTH FLOOR + toolchain close-out: commit the spec+plan · retrue "1 year"→"2 years" (Rail 02 + harvest + BACKLOG) · re-word the two-clock interim sentence · Safe-transfer checklist line · vendor OZ 5.x + Murky · pin `evm_version` · prove Halmos/slither/aderyn/mutation on THIS box (WSL fallback decided now) | Guards green · **corrected FR copy FULL TEXT on screen** · tools green on a sentinel | 🚀 BATCHABLE |
| **S3-1** ✅ (`f036419` — 36/36 first-run) | `MeritDistributor.sol` + unit/negative suite (byte-exact from the frozen spec; auto-close-on-drain adopted) | forge green · branch 100% money+access | ✅ NO DEPLOY |
| **S3-2** ✅ (`4b9a856`+`ffc2ccf` — invariants 8×50k zero-fail · mainnet-fork green · slither 0 high/med · chunk=50 measured; halmos/aderyn/mutation ride the GREEN close-out) | The adversarial stack: 15 invariants · Halmos · Murky fuzz · mutation ≥90% · slither+aderyn · fork tests · gas + empirical chunk | every §9 GREEN item except fixture+rehearsal | ✅ NO DEPLOY |
| **S3-3** ✅ (`7fe6cd7` — differential fixture GREEN both sides; guard 1,278 in the blocking chain) | Tooling: potPolicy · merkle · rulesHash · publishedFile · fixture (two-phase) · verify CLI · guard | golden leaf vectors byte-verified vs independent viem · differential test green | ✅ NO DEPLOY |
| **S3-4** ✅ | **THE ANTI-FARM LAWS** (§0.17-⑤): holding period · floor-gated credit · **NO referral cap (founder ruling 24-07, guard-pinned)** — DONE `1629ce6`+`94b03d9` | per-attack unit tests green · ANTI_FARM_IMPLEMENTED=true | 🚀 BATCHABLE |
| **S3-5** ◐ (S3-5a DONE `3e08eb4`: delta windows + the pot server twin + moneyContext; S3-5b = THE NEXT ACT) | Server dark-wiring: delta windows · seasonPotReadmodel (③b6a) + alarms · pot object DARK · own-row claimStatus · operator dispute route (server recompute) · interim-announcement registry · GOAL config · **member + founder bell kinds** | typecheck ×2 · api guards · fail-closed proof (payload without pot renders today's surfaces) · FR bell sentences on screen · guard-forbidden-copy + CANON_PROTOCOL_LANGUAGE §5 amended SAME commit for the new money words (§8-③, "the guard follows the system") | 🚀 BATCHABLE |
