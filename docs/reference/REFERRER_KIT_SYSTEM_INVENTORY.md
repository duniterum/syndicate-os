...
5. **Manual approval step between the two signatures**: *"Fail-closed check before activating: open /join?source=… — the quote must show NO referral line while the source is PAUSED. Once verified, sign the activation."* Also blocks activation if the on-chain metadataHash does not match the served terms document.
6. Founder signs the SECOND act `setSourceStatus(ACTIVE)` — the link goes live. REVOKED sources get no proposed act; sources are never deletable.
...
**What it CANNOT do**: create/activate/pause/revoke a source server-side (owner-only on-chain acts — only the registry owner's signature moves anything); `SourceReviewQueue` (in `AdminOperatorsCrud.tsx` line 466) approve/flag are DESIGN_PREVIEW, buttons disabled ("live approve/flag writes land with their Q42 slice"); the program kill-switch toggle in `AdminReferralCrud` is a labelled preview ("Save never activates or pauses the program: program state is an on-chain fact"); no lookup surface for arbitrary wallets' standing; no directory.
