...
function HolderIndexSection() {
  ...
  // The HEADLINE count reads the latest continuously re-verified run (the
  // same live line the public hero speaks — round-2 review 2026-08-02: this
  // card led with the frozen 2026-07-16 snapshot figure next to a live hero
  // at a different number). The sealed snapshot stays below as what it is:
  // the hash-pinned genesis attestation, an honest boot fallback.
  const live = useSpineAttestation();
    ...
    <section className="mb-12">
      <div className="mb-4">
        ...
        <p className="text-muted-foreground text-sm mt-1 measure">
          The headline count is the latest continuously re-verified membership figure — the same
          live line the home page speaks. The sealed card below it is the hash-pinned genesis
          attestation (a founder-gated offline build from the VERIFIED member-continuity build):
          counts and era boundaries only, no per-seat roster, no directory, no member-identifying
          data. Every aggregate carries its numbering-era provenance.
        </p>
      </div>
        ...
        <Card className="bg-card/40 border-border/50 p-6">
          <div className="flex flex-wrap items-baseline justify-between gap-4">
            <div>
              ...
              <div className="text-4xl font-light text-foreground mt-1">
                {live !== null ? live.memberTotal : data.memberTotal}
              </div>
              <p className="type-caption text-muted-foreground mt-1">
                {live !== null
                  ? `verified ${live.verifiedAtIso.slice(0, 10)} · matches the on-chain member count · re-checked continuously`
                  : `sealed snapshot figure — the live verification line was not readable just now`}
              </p>
            </div>
