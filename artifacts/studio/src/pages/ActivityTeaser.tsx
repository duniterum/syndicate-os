...
export default function ActivityTeaser() {
  return (
    <PublicPage
      ...
      lead="The protocol speaks through verified receipts. Seats, burns, referral events, liquidity, archive mints, treasury movements, milestone crossings and era turns are served by the event indexer — the complete on-chain record, newest first — refreshed live from the chain between cycles; Chronicle promotions join from the committed register. Every line names its act the way the chain wrote it — member number, short-form signature — and carries its own verify link to the full transaction. Never a claim, never more than the chain itself publishes."
