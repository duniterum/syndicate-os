...
- Wallet session shell (S2, dev-only): the app-wide access state is session-wired S1⇄S4 only (`WIRABLE_ACCESS_STATES`/`resolveWiredState` in `accessState.ts`; S1 stays the fail-closed boot default). The SIWE UI lives in `artifacts/studio/src/wallet/` and is reachable ONLY via the flag-gated dynamic import (`walletSessionGate.ts`: dev or `VITE_WALLET_SESSION_PREVIEW=true` build) — default production builds dead-code-eliminate it (guard-auth-zone dist-grep is the regression net). Session ≠ membership: no wallet→member lookup exists; server `/api/auth` is founder-locked and production-dark. Rerun `auth-skeleton:test` ≥60s apart (shared throttle).
