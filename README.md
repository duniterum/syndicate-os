...
- [Whole-system organism audit (2.20A)](docs/audits/SLICE_2_20A_WHOLE_SYSTEM_ORGANISM_AUDIT.md) — read-only senior audit of architecture harmony, truth/posture alignment, security/privacy, SEO, and the recommended repair/implementation sequence.
- [Post-convergence delta audit (2.20D)](docs/audits/SLICE_2_20D_POST_CONVERGENCE_DELTA_AUDIT.md) — read-only verification that the 2.20B/2.20C convergence closed the high-priority 2.20A findings, with remaining risks and the recommended next slice.
- [Duniterum capability harvest atlas (2.20E)](docs/strategy/DUNITERUM_CAPABILITY_HARVEST_2_20E.md) — read-only prior-art harvest across visible Duniterum repos (excluding Cesium): a cross-repo capability matrix, surface-by-surface feature harvest, rejected/unsafe patterns, and a prioritized integration backlog with the recommended spec-first next slice.
- [Principal prior-art audit: TheSyndicate (2.20F)](docs/strategy/PRINCIPAL_PRIOR_ART_AUDIT_THESYNDICATE_2_20F.md) — read-only deep audit of the principal ancestor `duniterum/TheSyndicate` across dimensions A–L: architecture/public-surface/canon map, member/operator/support/AI/economy findings, a path/count-only safety scan, an old-vs-new matrix, a PRESERVE/ADAPT/REWRITE/DEFER/REJECT preservation backlog, and the recommended next slices.
